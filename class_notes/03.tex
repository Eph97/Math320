...
\begin{definition}[Metric \& Metric Spaces]
	A function $d : X \times X \to \mathbb{R}$ on the Cartesian product of a set $X$ is a \underline{metric} if
...
A subset $V \subseteq X$ of a metric space $X$ is open if for every $x \in V$ there is a $\delta >0 $ such that $B(x,\delta) = \{y \in X: d(x,y) < \delta\} \subseteq U$ 
...
\begin{theorem}
	This is equivalent to $f$ being a measurable function from $(X, S_X)$ to $(Y, \sigma(T_Y))$
\end{theorem}
